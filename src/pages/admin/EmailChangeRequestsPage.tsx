...
export const EmailChangeRequestsPage: React.FC = () => {
  const [tab, setTab] = useState(0);
  const [page, setPage] = useState(1);
  const [requests, setRequests] = useState<EmailChangeRequestDto[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setErrorMsg("Failed to load email change requests.");
    } finally {
      setLoading(false);
    }
  }, [tab, page]);
  ...
  const handleApprove = async (request: EmailChangeRequestDto) => {
    setSaving(true);
    try {
      await approveEmailChangeRequest(request.id);
      showSuccess(`Approved — ${request.userName} is now ${request.requestedEmail}`);
      await load();
    } catch (err) {
  ...
  const handleRejectConfirm = async () => {
    ...
    setSaving(true);
    try {
      await rejectEmailChangeRequest(rejectTarget.id, rejectNotes.trim() || undefined);
  ...
  return (
    <Box>
      ...
      <Box sx={{ mb: 3 }}>
        <Typography variant="h4" sx={{ fontWeight: 700, mb: 0.5 }}>Email Change Requests</Typography>
        <Typography variant="body2" color="text.secondary">
          Nothing changes here until you approve it — any account's request to change its own email
          lands in this queue instead of taking effect immediately.
        </Typography>
      </Box>
          ...
          <Table>
            <TableHead>
              <TableRow sx={{ bgcolor: "#F8FAFC" }}>
                {["Account", "Change", "Reason", "Status", "Requested", "Actions"].map((col) => (
                ...
                requests.map((request) => {
                  ...
                  return (
                    <TableRow key={request.id} hover>
                      <TableCell>
                        <Typography variant="body2" sx={{ fontWeight: 600 }}>{request.userName}</Typography>
                        <Typography variant="caption" color="text.secondary">{request.currentEmail}</Typography>
                      </TableCell>
                      <TableCell>
                        <Typography variant="body2">→ <strong>{request.requestedEmail}</strong></Typography>
                      </TableCell>
      ...
      <Dialog open={!!rejectTarget} onClose={() => setRejectTarget(null)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <DialogContentText sx={{ mb: 2 }}>
            Rejecting <strong>{rejectTarget?.userName}</strong>'s request to change their email to{" "}
            <strong>{rejectTarget?.requestedEmail}</strong>.
          </DialogContentText>
          <TextField
            fullWidth
            multiline
...
export default EmailChangeRequestsPage;
